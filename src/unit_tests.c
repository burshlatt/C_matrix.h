...
START_TEST(test_s21_create_matrix_1) {
  matrix_t A = {0};
  int fail_value = s21_create_matrix(-1, 1, &A);
  ck_assert_int_eq(fail_value, 1);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(test_s21_calc_complements_9) {
  ...
  int fail1 = s21_calc_complements(&A, &result);
...
START_TEST(test_s21_inverse_matrix_1) {
  ...
  int fail1 = s21_inverse_matrix(&A, &result);

  ck_assert_double_eq(fail1, 2);
...
START_TEST(test_s21_inverse_matrix_2) {
  ...
  A.matrix[2][1] = 2;
  A.matrix[2][2] = 6;
  int fail2 = s21_inverse_matrix(&A, &result);
...
int main() {
  ...
  tcase_add_test(tc1_1, test_s21_mult_matrix_7);
  ...
  tcase_add_test(tc1_1, test_s21_calc_complements_8);
  tcase_add_test(tc1_1, test_s21_calc_complements_9);
